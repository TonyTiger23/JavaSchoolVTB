...
public class Cat implements Runnable, Jumpable{
    ...
    Cat(String name, int max_height, int max_distance) {
        this.name = name;
    ...
    public boolean run(int distance) {
        if (distance > max_distance) {
            System.out.println("Кот "+ name+  " устал и пробежал только " + max_distance + " метров");
            return false;
        }
        else
        {
    ...
    public boolean jump(int height) {
        if (height > max_height) {
            System.out.println("Кот "+ name+  " устал и прыгнул только на" + max_height + " метров");
            return false;
        }
        else
        {
